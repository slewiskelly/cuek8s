...
func New() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "apply [FLAGS] INPUT...",
		Short: "Deploys Kubernetes resources.",
		Long: `Deploys Kubernetes resources.

Input(s) are the same as those of the ` + "`cue`" + ` tool. See ` + "`cue inputs`" + ` for more information.

If specified, -d or --deliverables will deploy resources for the specific deliverable(s).
`,
...
func run(args []string, opts *options) error {
	...
	for it.Next() {
		...
		err := flow.New(&flow.Config{Root: cue.ParsePath(fmt.Sprintf("Delivery.%s.apply", it.Selector().String()))}, v, tasks.Func).Run(context.Background())
		if err != nil {
			return err
		}
	}
